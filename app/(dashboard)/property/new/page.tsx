...
export default function NewPropertyPageWrapper() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><div className="text-earth-muted font-semibold">Loading...</div></div>}>
      <NewPropertyPage />
    </Suspense>
  )
}
      ...
      <header className="px-8 py-4 border-b border-earth-border bg-white/90 backdrop-blur-[12px] sticky top-0 z-30">
        <div className="max-w-[800px] mx-auto flex items-center justify-between">
          <Link href="/dashboard" className="text-xl font-extrabold text-earth-dark tracking-tight no-underline flex items-center gap-2">
            ...
            <span className="text-earth-dark font-serif">HeyConcierge</span>
          </Link>
        ...
        <div className="flex items-center gap-2 mb-2">
          {steps.map((_, i) => (
            <div key={i} className="flex-1 flex items-center gap-2">
              <div className={`h-2 rounded-full flex-1 transition-all ${i + 1 <= step ? 'bg-grove' : 'bg-slate-200'}`} />
            </div>
          ))}
        </div>
      ...
      <div className="max-w-[600px] mx-auto w-full px-8 flex-1 pb-12">
        ...
        {step === 1 && (
          <div className="animate-slide-up">
            <div className="flex items-start justify-between mb-8">
              <div>
                <h2 className="text-3xl font-extrabold text-earth-dark tracking-tight mb-2">Add a property</h2>
                <p className="text-earth-muted">Tell us about your property.</p>
              </div>
              <div className="flex items-center gap-3 bg-white rounded-xl border border-earth-border px-5 py-3">
                ...
                <div className="text-xs">
                  <p className="font-bold text-earth-dark">Step {step}: Property</p>
                  <p className="text-earth-muted">{steps.slice(step).join(' \u2192 ')}</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-start justify-between mb-8">
              ...
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-3 bg-white rounded-xl border border-earth-border px-5 py-3">
                  ...
                  <div className="text-xs">
                    <p className="font-bold text-earth-dark">Step {step}: Config</p>
                    <p className="text-earth-muted">{steps.slice(step).join(' \u2192 ')}</p>
                  </div>
                </div>
                {hasGuestKnowledge && (
        ...
        {step === 3 && (
          <div className="animate-slide-up text-center">
            ...
            <h2 className="text-3xl font-extrabold text-earth-dark tracking-tight mb-2">Property added!</h2>
            <p className="text-earth-muted mb-8">Your AI concierge is ready. Place this QR code in your property &mdash; guests scan it to open Telegram and start chatting.</p>
            {qrDataUrl && (
              <div className="inline-block bg-white rounded-3xl p-8 border border-earth-border shadow-sm mb-8">
                ...
                <p className="mt-4 font-serif text-earth-dark">{form.propertyName}</p>
            ...
            <div className="flex gap-4 justify-center flex-wrap">
              ...
              {createdPropertyId && (
                <Link href={`/property/${createdPropertyId}/settings`} className="inline-flex items-center gap-2 border-2 border-grove text-grove px-8 py-3 rounded-full font-semibold no-underline transition-all hover:-translate-y-0.5">
                  Property Settings
                </Link>
              )}
              {qrDataUrl && (
        ...
        {step < 3 && (
          <div className="flex justify-between mt-10">
            {step === 1 ? (
              <Link href="/dashboard" className="text-earth-muted font-bold no-underline hover:text-grove transition-colors">&#8592; Dashboard</Link>
            ) : (
              <button onClick={() => setStep(s => s - 1)} className="text-earth-muted font-bold hover:text-grove transition-colors">
                &#8592; Back
              </button>
            )}
            <button
              ...
              className={`px-8 py-3 rounded-full font-semibold text-white transition-all ${canNext() ? 'bg-grove hover:-translate-y-0.5 shadow-[0_4px_15px_rgba(108,92,231,0.3)]' : 'bg-slate-300 cursor-not-allowed'}`}
            >
  ...
  return (
    <div>
      ...
      <input
        ...
        className="w-full px-4 py-3 rounded-xl border-2 border-earth-border bg-white text-earth-dark font-medium placeholder:text-earth-light focus:border-grove focus:outline-none transition-colors"
      />
    </div>
  )
}
...
function TextArea({ value, onChange, placeholder }: { value: string; onChange: (v: string) => void; placeholder: string }) {
  return (
    <textarea
      ...
      placeholder={placeholder}
      rows={3}
      className="w-full px-4 py-3 rounded-xl border-2 border-earth-border bg-white text-earth-dark font-medium placeholder:text-earth-light focus:border-grove focus:outline-none transition-colors resize-none"
    />
  )
}
