...
function UpsellPage() {
  ...
  const inputClass = "w-full px-4 py-3 rounded-xl border-2 border-earth-border focus:border-grove outline-none transition-all bg-white text-sm"
  const toggleClass = (enabled: boolean) =>
    `relative inline-flex h-7 w-12 items-center rounded-full transition-colors cursor-pointer ${
      enabled ? 'bg-grove' : 'bg-[rgba(108,92,231,0.15)]'
    }`
  ...
  return (
    <div className="min-h-screen">
      {/* Unsaved changes bar */}
      {hasUnsavedChanges && (
        <div className="sticky top-0 z-40 bg-gradient-to-r from-[#FDCB6E] to-[#F9CA24] px-4 py-2 text-center text-sm font-bold text-earth-dark">
      ...
      {/* Header */}
      <header className={`px-4 sm:px-8 py-4 border-b border-earth-border bg-white/90 backdrop-blur-[12px] sticky ${hasUnsavedChanges ? 'top-[36px]' : 'top-0'} z-30`}>
        <div className="max-w-6xl mx-auto flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            <Link href="/dashboard" className="text-xs sm:text-sm text-earth-dark hover:text-grove font-bold whitespace-nowrap">
              🏠 <span className="hidden sm:inline">Dashboard</span>
            </Link>
            <Link href="/calendar" className="text-xs sm:text-sm text-earth-dark hover:text-grove font-bold whitespace-nowrap">
              📅 <span className="hidden sm:inline">Calendar</span>
            </Link>
            <span className="text-xs sm:text-sm text-grove font-bold whitespace-nowrap">
            ...
            <div className="relative">
              <button
                ...
                className="text-xs sm:text-sm font-bold text-earth-dark hover:text-grove flex items-center gap-1 max-w-[120px] sm:max-w-none truncate"
              >
              ...
              {dropdownOpen && (
                <div className="absolute right-0 mt-2 w-64 bg-white rounded-xl shadow-lg border border-earth-border py-2 z-50">
                  <div className="px-4 py-3 border-b border-earth-border">
                    <div className="text-xs text-earth-muted uppercase font-bold mb-1">Current Plan</div>
                    <div className="text-sm font-bold text-grove">{organization?.plan || 'FREE'}</div>
                  </div>
                  <button
                    onClick={() => { setDropdownOpen(false); handleLogout() }}
                    className="w-full text-left px-4 py-3 text-sm text-earth-muted hover:bg-grove/[0.04] transition-colors border-t border-earth-border"
                  >
                    🚪 Logout
                  </button>
                </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-8 py-8 sm:py-12">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-sm sm:text-base text-earth-muted">Boost revenue with automated offers to your guests</p>
          </div>
          ...
          {properties.length > 1 && (
            <select
              ...
              className="bg-white px-4 py-3 rounded-xl border-2 border-earth-border focus:border-grove outline-none font-bold text-sm"
            >
              {properties.map(p => (
        ...
        {properties.length === 0 ? (
          <div className="bg-white rounded-xl border border-earth-border p-12 text-center">
            ...
            <h2 className="text-2xl font-extrabold text-earth-dark tracking-tight mb-2">No properties yet</h2>
            <p className="text-earth-muted mb-6">Add a property first to start upselling</p>
            <Link href="/property/new" className="bg-grove text-white px-6 py-3 rounded-full font-bold no-underline hover:-translate-y-0.5 transition-all">
              + Add Property
            </Link>
          </div>
        ) : (
            ...
            <div className="flex gap-2 mb-8">
              <button
                ...
                className={`px-6 py-3 rounded-full font-bold text-sm transition-all ${
                  activeTab === 'dashboard'
                    ? 'bg-grove text-white shadow-md'
                    : 'bg-white text-earth-dark hover:bg-grove/[0.04] border-2 border-earth-border'
                }`}
              >
                📊 Dashboard
              </button>
    ...
    <div className="space-y-8">
      ...
      {config && !config.enabled && (
        <div className="bg-yellow/10 border-2 border-yellow/30 rounded-2xl px-6 py-4 flex items-center gap-3">
          ...
          <div>
            <p className="font-bold text-earth-dark text-sm">Upselling is paused for {selectedProperty?.name}</p>
            <p className="text-xs text-earth-muted">Go to Settings to enable automated offers</p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <StatCard emoji="💰" value={`€${stats.revenue}`} label="Revenue" color="text-grove" large />
      </div>
      ...
      <div className="bg-white rounded-xl border border-earth-border overflow-hidden">
        <div className="px-6 sm:px-8 py-5 bg-gradient-to-r from-[rgba(108,92,231,0.06)] to-transparent">
          <h2 className="font-bold text-earth-dark text-lg">📋 Offer Breakdown</h2>
        </div>
        <div className="px-6 sm:px-8 py-6">
              ...
              return (
                <div key={type} className="bg-[rgba(108,92,231,0.03)] rounded-2xl p-4 border border-[rgba(108,92,231,0.06)]">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="font-bold text-sm text-earth-dark">{label}</span>
                  </div>
                  <div className="grid grid-cols-3 gap-2 text-center">
                    <div>
                      <div className="text-lg font-black text-earth-dark">{sent}</div>
                      <div className="text-[10px] text-earth-muted uppercase font-bold">Sent</div>
                    </div>
                    <div>
                      <div className="text-lg font-black text-mint-dark">{accepted}</div>
                      <div className="text-[10px] text-earth-muted uppercase font-bold">Accepted</div>
                    </div>
                    <div>
                      <div className="text-lg font-black text-grove">€{revenue}</div>
                      <div className="text-[10px] text-earth-muted uppercase font-bold">Revenue</div>
                    </div>
                  </div>
                </div>
              )
      ...
      <div className="bg-white rounded-xl border border-earth-border overflow-hidden">
        <div className="px-6 sm:px-8 py-5 bg-gradient-to-r from-[rgba(108,92,231,0.06)] to-transparent flex items-center justify-between">
          <h2 className="font-bold text-earth-dark text-lg">📬 Recent Offers</h2>
          <span className="text-xs text-earth-muted font-bold">{offers.length} total</span>
        </div>

        {offers.length === 0 ? (
          <div className="px-8 py-12 text-center">
            <div className="text-4xl mb-3">📭</div>
            <p className="text-earth-muted font-bold">No offers yet</p>
            <p className="text-xs text-earth-muted mt-1">Offers will appear here once upselling is enabled and bookings are synced</p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-earth-border">
                  <th className="text-left px-6 py-3 text-xs text-earth-muted font-bold uppercase">Type</th>
                  <th className="text-left px-4 py-3 text-xs text-earth-muted font-bold uppercase">Guest</th>
                  <th className="text-left px-4 py-3 text-xs text-earth-muted font-bold uppercase">Channel</th>
                  <th className="text-left px-4 py-3 text-xs text-earth-muted font-bold uppercase">Price</th>
                  <th className="text-left px-4 py-3 text-xs text-earth-muted font-bold uppercase">Status</th>
                  <th className="text-left px-4 py-3 text-xs text-earth-muted font-bold uppercase">Date</th>
                </tr>
              </thead>
              <tbody>
                {offers.slice(0, 20).map(offer => {
                  ...
                  return (
                    <tr key={offer.id} className="border-b border-[rgba(108,92,231,0.04)] hover:bg-[rgba(108,92,231,0.02)]">
                      ...
                      <td className="px-4 py-3 text-earth-muted">
                      ...
                      <td className="px-4 py-3 text-earth-muted text-xs">
                        {offer.sent_at
...
function StatCard({ emoji, value, label, color, large }: {
  ...
  return (
    <div className="bg-white rounded-xl border border-earth-border p-5 sm:p-6">
      <div className="text-2xl sm:text-3xl mb-2">{emoji}</div>
      <div className={`${large ? 'text-2xl sm:text-3xl' : 'text-xl sm:text-2xl'} font-black ${color || 'text-earth-dark'}`}>
        {value}
      </div>
      <div className="text-[11px] text-earth-muted font-bold uppercase mt-1">{label}</div>
    </div>
  )
}
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-earth-border overflow-hidden">
        <div className="px-6 sm:px-8 py-6 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="font-bold text-earth-dark text-lg">Automated Upselling</h2>
              <p className="text-xs text-earth-muted mt-0.5">When enabled, offers will be sent to guests automatically</p>
            </div>
          </div>
          <button
        ...
        <div className="px-6 sm:px-8 py-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-earth-muted">Send offers automatically, or require your approval first</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <span className={`text-xs font-bold ${!config.auto_send ? 'text-grove' : 'text-earth-muted'}`}>Draft</span>
            <button
            ...
            <span className={`text-xs font-bold ${config.auto_send ? 'text-grove' : 'text-earth-muted'}`}>Auto</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-earth-border overflow-hidden">
        <div className="px-6 sm:px-8 py-5 flex items-center justify-between">
          ...
          <select
            ...
            className="px-4 py-2 rounded-xl border-2 border-earth-border focus:border-grove outline-none text-sm font-bold bg-white"
          >
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="text-xs font-bold text-earth-muted uppercase mb-1 block">Price per hour (€)</label>
            <input
              type="number"
          ...
          <div>
            <label className="text-xs font-bold text-earth-muted uppercase mb-1 block">Max extra hours</label>
            <input
              type="number"
          ...
          <div>
            <label className="text-xs font-bold text-earth-muted uppercase mb-1 block">Standard checkout time</label>
            <input
              type="time"
          ...
          <div>
            <label className="text-xs font-bold text-earth-muted uppercase mb-1 block">Send offer X hours before checkout</label>
            <input
              type="number"
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            <label className="text-xs font-bold text-earth-muted uppercase mb-1 block">Max early hours</label>
            <input
              type="number"
          ...
          <div>
            <label className="text-xs font-bold text-earth-muted uppercase mb-1 block">Standard check-in time</label>
            <input
              type="time"
          ...
          <div>
            <label className="text-xs font-bold text-earth-muted uppercase mb-1 block">Send offer X hours before check-in</label>
            <input
              type="number"
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="text-xs font-bold text-earth-muted uppercase mb-1 block">Base price per night (€)</label>
            <input
              type="number"
          ...
          <div>
            <label className="text-xs font-bold text-earth-muted uppercase mb-1 block">Discount (%)</label>
            <input
              type="number"
          ...
          <div>
            <label className="text-xs font-bold text-earth-muted uppercase mb-1 block">Max gap nights</label>
            <input
              type="number"
          ...
          <div>
            <label className="text-xs font-bold text-earth-muted uppercase mb-1 block">Send X days before checkout</label>
            <input
              type="number"
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            <label className="text-xs font-bold text-earth-muted uppercase mb-1 block">Send X hours before checkout</label>
            <input
              type="number"
      ...
      >
        <div className="grid grid-cols-1 gap-4">
          <div>
            <label className="text-xs font-bold text-earth-muted uppercase mb-1 block">Send X hours after checkout</label>
            <input
              type="number"
          ...
          <div>
            <label className="text-xs font-bold text-earth-muted uppercase mb-1 block">Google Review URL</label>
            <input
              type="url"
          ...
          <div>
            <label className="text-xs font-bold text-earth-muted uppercase mb-1 block">Airbnb Review URL</label>
            <input
              type="url"
          ...
          <div>
            <label className="text-xs font-bold text-earth-muted uppercase mb-1 block">TripAdvisor Review URL</label>
            <input
              type="url"
      ...
      <div className="flex justify-end pt-4">
        <button
          onClick={handleSave}
          disabled={saving}
          className="bg-grove text-white px-8 py-4 rounded-full font-semibold text-lg hover:-translate-y-0.5 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg"
        >
...
function OfferSection({
  ...
  return (
    <div className={`bg-white rounded-xl border border-earth-border overflow-hidden transition-all ${!enabled ? 'opacity-75' : ''}`}>
      {/* Header with toggle */}
      <div className="px-6 sm:px-8 py-5 bg-gradient-to-r from-[rgba(108,92,231,0.06)] to-transparent flex items-center justify-between">
        <div className="flex items-center gap-3">
          <span className="text-2xl">{emoji}</span>
          <div>
            <h3 className="font-bold text-earth-dark text-base">{title}</h3>
            <p className="text-[11px] text-earth-muted">{description}</p>
          </div>
        </div>
...
function OfferPreview({ children }: { children: React.ReactNode }) {
  return (
    <div className="mt-4 bg-[rgba(108,92,231,0.04)] rounded-xl p-4 border border-dashed border-[rgba(108,92,231,0.15)]">
      <div className="text-[10px] text-earth-muted font-bold uppercase mb-2">Message Preview</div>
      <div className="text-sm text-earth-dark leading-relaxed">
        {children}
      </div>
    </div>
  )
