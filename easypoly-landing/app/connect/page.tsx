...
export default function ConnectPage() {
  ...
  useEffect(() => {
    ...
    const userId = params.get('userId') || params.get('user_id') || (window as any).Telegram?.WebApp?.initDataUnsafe?.user?.id;
  ...
  async function connectWallet() {
    try {
      ...
      // Derive Polymarket CLOB API credentials client-side using the wallet signer
      // This is secure because the private key never leaves MetaMask
      const clobClient = new ClobClient(
        'https://clob.polymarket.com',
        137, // Polygon chain ID
        signer as any // Pass the MetaMask signer
      );

      // Derive API credentials - this will prompt the user to sign in MetaMask
      const credentials = await clobClient.deriveApiKey();

      const apiKey = credentials.apiKey;
      const apiSecret = credentials.secret;
      const apiPassphrase = credentials.passphrase;
        ...
        {status === 'idle' && (
          <div className="space-y-4">
            <p className="text-sm text-white/70">
              ...
              Your funds stay in your wallet — we never hold your money.
            </p>
            {!telegramUserId && (
