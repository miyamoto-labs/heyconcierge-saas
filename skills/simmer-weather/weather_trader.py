...
def fetch_json(url, headers=None):
    """Fetch JSON from URL using curl (bypasses Python SSL/LibreSSL issues)."""
    import subprocess
    try:
        cmd = ["curl", "-s", "--max-time", "15", "-H", "Accept: application/json"]
        for k, v in (headers or {}).items():
            cmd.extend(["-H", f"{k}: {v}"])
        cmd.append(url)
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=20)
        if result.returncode != 0:
            print(f"  Curl error ({result.returncode}): {url}")
            return None
...
def sdk_request(api_key: str, method: str, endpoint: str, data: dict = None) -> dict:
    ...
    try:
        ...
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=20)
        if not result.stdout.strip():
            return {"error": f"Empty response from {endpoint}"}
        return json.loads(result.stdout)
    except subprocess.TimeoutExpired:
